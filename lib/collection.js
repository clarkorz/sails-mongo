...
Collection.prototype.update = function update(criteria, values, cb) {
  ...
  // Optimize for updating a simgle document which explicit _id.
  // This will proform an atom operation.
  if (_.has(query.criteria.where, '_id') && query.criteria.where._id instanceof ObjectId) {
    return collection.findAndModify(query.criteria.where, {}, { '$set': values }, { new: true }, function(err, record) {
      if(err) return cb(err);
      cb(null, utils.rewriteIds(record ? [record] : [], self.schema));
    });
  }

  // Lookup records being updated and grab their ID's
  // Useful for later looking up the record after an update
  ...
  collection.find(query.criteria.where, { _id: 1 }).toArray(function(err, records) {
    ...
    var updatedRecords = _.pluck(records, '_id');
...
Collection.prototype.destroy = function destroy(criteria, cb) {
  ...
  // Optimize for deleting a simgle document which is specified by explicit _id.
  // This will proform an atom operation.
  if (_.has(query.criteria.where, '_id') && query.criteria.where._id instanceof ObjectId) {
    return collection.findAndRemove(query.criteria.where, { _id: -1 }, function(err, record) {
      if(err) return cb(err);
      cb(null, utils.rewriteIds(record ? [record] : [], self.schema));
    });
  }

  // Lookup records being deleted and grab their ID's
  // Useful for later looking up the record after an remove
  // Required because options may not contain an ID
  collection.find(query.criteria.where).toArray(function(err, records) {
    if(err) return cb(err);
    if(!records) return cb(Errors.NotFound);

    // Build an array of _ids of records
    var deletedRecords = _.pluck(records, '_id');

    collection.remove({ _id: { $in: deletedRecords } }, function(err, result) {
      if(err) return cb(err);
      cb(null, utils.rewriteIds(records, self.schema));
    });
  });
