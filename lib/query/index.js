  ...
  Object.keys(obj).forEach(function(attr) {
    ...
    if(_.isPlainObject(obj[attr]) && !_.isDate(obj[attr])) {
      ...
      if((attr !== '!' && attr !== 'not') || Object.keys(obj[attr]).length > 1) {
        self.parseOptions(attr, obj[attr], original, attr);
        return;
      }
    ...
    // If the schema says it's an `objectid`, then try to convert it.
    if(hop(self.schema, parent) && self.schema[parent].type === 'objectid') {
      val = obj[attr];
      obj[attr] = utils.matchMongoId(val) ? new ObjectId(val) : val;

      // Check if the attribute is embed, and if so then turn it into sub-document query
      if(self.schema[parent].embed) {
        original[parent + '._id'] = original[parent];
    ...
    if(utils.matchMongoId(obj[attr])) {
      ...
      // Check if the attribute is embed, and if so then turn it into sub-document query
      if(hop(self.schema, attr) && self.schema[attr].embed) {
        original[attr + '._id'] = obj[attr];
        delete original[attr];
      }
